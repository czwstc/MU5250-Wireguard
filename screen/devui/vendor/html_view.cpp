...
#ifdef __APPLE__
    return (long long)st.st_mtime * 1000000000LL + (long long)st.st_mtimespec.tv_nsec;
#else
...
extern "C" void html_view_init(uint16_t *fb, int w, int h, int pitch_px, int rotate, const char *font_path)
{
    ...
    (void)font_path;
    if (FT_Init_FreeType(&g_ft) || FT_New_Memory_Face(g_ft, devui_font, sizeof(devui_font), 0, &g_face)) {
        fprintf(stderr, "html_view: cannot load embedded OFL font\n");
        std::abort();
    }
