...
class Transactions(unittest.TestCase):
    ...
    def test_screen_binary_and_gate_are_restored_together(self):
        self.write('data/bin/openui-screen', 'old screen')
        self.write('data/local/tmp/openui-screen-verified', 'old verification')
        self.action('begin')
        self.write('data/bin/openui-screen', 'new screen')
        (self.root / 'data/local/tmp/openui-screen-verified').unlink()
        self.action('restore')
        self.assertEqual((self.root / 'data/bin/openui-screen').read_text(), 'old screen')
        self.assertEqual((self.root / 'data/local/tmp/openui-screen-verified').read_text(), 'old verification')

    def test_legacy_snapshot_does_not_delete_untracked_new_components(self):
        self.action('begin')
        # Historical snapshots have no per-snapshot target inventory.
        (self.root / 'data/local/tmp/open-u60-transactions/test-123/targets').unlink()
        self.write('data/bin/openui-screen', 'new component not tracked by old snapshot')
        self.write('data/bin/wg', 'existing tools')
        self.action('restore')
        self.assertTrue((self.root / 'data/bin/openui-screen').exists())
        self.assertEqual((self.root / 'data/bin/wg').read_text(), 'existing tools')
