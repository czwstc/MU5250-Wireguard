...
const labels: Record<string, string> = { idle: 'Stock UI', starting: 'Opening', active: 'DevUI active', restoring: 'Restoring', error: 'Check recovery status' }
export default function ScreenControl() {
  ...
  return <Card title="DevUI device screen" action={<Chip tone={status?.state === 'error' ? 'warn' : status?.running ? 'accent' : 'default'}>{status ? labels[status.state] ?? status.state : 'Loading'}</Chip>}>
    <div className="space-y-3">
      <p className="text-xs leading-relaxed text-ink2">Open the DevUI menu for device status, WireGuard controls, saved profiles and device routing. Exit or leave it idle for two minutes to restore the stock UI. Saved routing rules stay active.</p>
