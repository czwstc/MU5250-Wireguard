...
data/bin/wg
data/bin/openui-screen
data/local/tmp/openui-screen-verified
data/local/tmp/openui-screen-licenses"
...
discard-incomplete)
    ...
    ;;
restore|complete)
    if [ -f "$transaction/targets" ]; then targets=$(cat "$transaction/targets"); else targets=$legacy_targets; fi
    test "$(cat "$transaction/identity")" = "$identity"
