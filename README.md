...
![Five saved WireGuard configurations (demo data)](docs/images/wireguard-profiles.png)
...
The agent and dashboard were deployed to the target B31 device on 2026-09-13.
Verification checked artifact hashes, existing login settings, the five-profile
API, legacy configuration migration and unchanged saved device routing. The
existing screen binary was byte-identical and retained its recovery verification.
WireGuard remained off during deployment; live VPN egress was not tested in this
update. Local Rust/Python/frontend tests, native rendering and profile UI checks
also passed. Deployment recovery snapshots are retained on the device.
